...
def run_git_diff():
    ...
    if not source_branch or not target_branch:
        raise ValueError("Environment variables CI_MERGE_REQUEST_SOURCE_BRANCH_NAME and CI_MERGE_REQUEST_TARGET_BRANCH_NAME must be set")

    print("Running git diff command...")
    result = subprocess.run(['git', 'diff', '-U0', f'remotes/origin/{target_branch}', f'remotes/origin/{source_branch}', '--', '*.java'], capture_output=True, text=True)
...
def run_git_diff_for_changed_files_names():
    ...
    result = subprocess.run(['git', 'diff', '--name-only', f'remotes/origin/{target_branch}', f'remotes/origin/{source_branch}', '--', '*.java'], capture_output=True, text=True)
    print("Git diff command output:\n", result.stdout)
